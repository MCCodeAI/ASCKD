...
import { Avatar, Button, Checkbox, Divider, Dropdown, Flex, type GetProp, message, Pagination, Space, theme } from 'antd';
...
import { useMarkdownTheme } from '../../utils/theme';
...
const SEARCH_ENGINES = [
    { key: 'google', label: 'Google', icon: <GlobalOutlined /> },
    { key: 'bing', label: 'Bing', icon: <SearchOutlined /> },
    { key: 'duckduckgo', label: 'DuckDuckGo', icon: <SearchOutlined /> },
    { key: 'baidu', label: 'Baidu', icon: <SearchOutlined /> },
];
...
const providerFactory = (conversationKey: string, searchEngines: string[], model: string) => {
    const cacheKey = `${conversationKey}-${searchEngines.join(',')}-${model}`;

    if (!providerCaches.get(cacheKey)) {
        // @ts-ignore: Custom params are supported by the backend but not strictly typed in SDK
        providerCaches.set(
            cacheKey,
    ...
    return providerCaches.get(cacheKey) as OpenAIChatProvider;
};
...
// Custom Link Component to handle new window and port mapping
const Link = (props: React.AnchorHTMLAttributes<HTMLAnchorElement>) => {
    const { href, children, ...rest } = props;
    let targetHref = href;

    // Map backend port (8000) to frontend port (current origin) for docs
    // Check if it points to localhost:8000 (with or without /docs, though we removed /docs prefix now)
    if (href && href.includes('localhost:8000')) {
        // Replace http://localhost:8000 with current origin (e.g., http://localhost:5173)
        // This allows the proxy to handle the request
        try {
            const url = new URL(href);
            // Reconstruct URL with current origin
            targetHref = `${window.location.origin}${url.pathname}${url.search}${url.hash}`;
        } catch (e) {
    ...
    return (
        <a
            ...
            {...rest}
            onClick={(e) => {
                // Stop propagation to prevent bubble click events if any
                e.stopPropagation();
            }}
        >
            {children}
...
const Independent: React.FC = () => {
    ...
    const [selectedSearchEngines, setSelectedSearchEngines] = useState<string[]>(['duckduckgo']);
    const [selectedModel, setSelectedModel] = useState<string>('gpt-4o-mini');

    // ==================== Runtime ====================

    const { onRequest, messages, isRequesting, abort, onReload, setMessage } = useXChat<ChatMessage>({
        provider: providerFactory(activeConversationKey, selectedSearchEngines, selectedModel), // every conversation has its own provider
    ...
    const searchEngineMenu: MenuProps = {
        items: SEARCH_ENGINES.map((engine) => ({
            key: engine.key,
            label: (
                <Checkbox
                    checked={selectedSearchEngines.includes(engine.key)}
                    onChange={(e) => {
                        if (e.target.checked) {
                            setSelectedSearchEngines([...selectedSearchEngines, engine.key]);
                        } else {
                            setSelectedSearchEngines(selectedSearchEngines.filter(k => k !== engine.key));
                        }
                    }}
                >
                    {engine.label}
    ...
    const saveRecentQuestion = (question: string) => {
        const newQuestions = [question, ...recentQuestions.filter(q => q !== question)].slice(0, 7);
        setRecentQuestions(newQuestions);
        localStorage.setItem('recentQuestions', JSON.stringify(newQuestions));
    };
    ...
    const chatSender = (
        ...
        >
            {/* 🌟 Prompts */}
            {!attachmentsOpen && !isInputFocused && (
                <Prompts
            ...
            {/* Wrapper for Input and Recent Questions to constrain width */}
            <div style={{ width: '100%', maxWidth: 840, position: 'relative' }}>
                {/* Recent Questions List */}
                {isInputFocused && recentQuestions.length > 0 && (
                    <div
                        style={{
                            position: 'absolute',
                            bottom: '100%',
                    ...
                    >
                        <div style={{ padding: '4px 12px', color: token.colorTextDescription, fontSize: 12 }}>Recent Questions</div>
                ...
                <Sender
                    ...
                    onBlur={() => {
                        // Small delay to allow click on list item to register if onMouseDown doesn't catch it
                        // But onMouseDown with preventDefault is better.
                        // Just setting false here is fine if we use onMouseDown.
                        setIsInputFocused(false);
                    }}
                    prefix={
                        <Button
                            ...
                            <Flex justify="space-between" align="center">
                                <Flex gap="small" align="center">
                                    ...
                                    <Button
                                        ...
                                        icon={
                                            <GlobalOutlined
                                                style={{
                                                    fontSize: 16,
                                                    color: webSearchEnabled ? '#1890ff' : undefined
                                                }}
                                            />
                                        }
                                        onClick={() => {
                                            ...
                                            if (newValue) {
                                                messageApi.info(`Web Search enabled with: ${selectedSearchEngines.join(', ')}`);
                                            } else {
                                                messageApi.info('Web Search disabled');
                                            }
                                        }}
                                    />
                                    ...
                                    <Dropdown
                                        ...
                                        trigger={['hover']}
                                    >
                                        <Sender.Switch value={false} icon={<AppstoreAddOutlined />}>
                                            Search Engines
                                        </Sender.Switch>
                                    </Dropdown>
                                </Flex>
